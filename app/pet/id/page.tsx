...
export default function PetDetay() {
  ...
  if (!pet) return <div className="text-center p-10 font-bold">Pati bulunamadı!</div>;
    ...
    <main className="flex min-h-screen flex-col items-center p-4 bg-gradient-to-b from-amber-50 to-orange-100 font-sans">
      ...
      <div className="bg-white p-4 rounded-[3rem] shadow-2xl border-8 border-white w-full max-w-sm overflow-hidden">
        ...
        <div className="text-center space-y-2 mb-6">
          ...
          <div className="inline-block bg-amber-100 px-4 py-1 rounded-full text-amber-600 font-black text-sm uppercase">
            {pet.puan || 0} Toplam Puan
          </div>
        </div>

        <button 
          onClick={oyVer}
          className="w-full py-5 rounded-2xl bg-gradient-to-r from-orange-500 to-amber-500 text-white font-black text-xl shadow-lg active:scale-95 transition-all uppercase italic"
        >
          Bu Patiye Oy Ver! 🐾
        </button>
      </div>
