...
export default function Home() {
  ...
  const [secilenPuan, setSecilenPuan] = useState<number>(5); 
  ...
  const oyVer = async (etiket: string) => {
    ...
    // Opsiyonel: Burada oylama loglarını tutacağın bir tabloya 'etiket' verisini gönderebilirsin.
        ...
        <div className="w-full max-w-xl flex flex-col items-center gap-2">
          <div className="w-full flex items-center justify-between bg-white/10 backdrop-blur-2xl border border-white/10 p-3 rounded-[2.5rem] shadow-2xl pointer-events-auto">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={cikisYap} className="bg-red-500/10 p-2.5 rounded-full border border-red-500/10 text-red-500 hover:bg-red-500/20 transition-all active:scale-90">
          ...
          <div className="bg-amber-500 px-6 py-1 rounded-full shadow-[0_5px_20px_rgba(245,158,11,0.4)] text-[10px] font-black italic text-black uppercase pointer-events-auto border border-amber-600 animate-pulse">
        ...
        <section key={foto.id + index} ref={fotolar.length === index + 1 ? sonElemanRef : null} className="h-screen w-full relative flex items-center justify-center snap-start snap-always bg-zinc-900">
          ...
          <div className="relative w-full h-full flex items-center justify-center p-4 pt-24">
            ...
            <div className="absolute right-4 bottom-24 flex flex-col items-center gap-6">
              <button onClick={() => paylas(foto.foto_url)} className="p-4 rounded-full bg-white/5 backdrop-blur-md border border-white/10 text-white hover:bg-white/20 active:scale-90 shadow-xl transition-all">
            ...
            {/* SOL PANEL: SIRADAKİ PET */}
            <button onClick={sonrakiPet} className="absolute left-6 bottom-24 p-4 rounded-full bg-white/5 border border-white/10 text-amber-500 animate-bounce active:scale-95 transition-all shadow-xl">
      ...
      {oylamaPaneli.open && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/80 backdrop-blur-xl animate-in fade-in duration-300">
          <div className="bg-zinc-900 border border-white/10 w-full max-w-sm p-8 rounded-[3.5rem] shadow-2xl">
            ...
            <h3 className="text-white text-center font-black italic uppercase text-lg mb-1 tracking-tighter">Puan Ver</h3>
            <p className="text-white/40 text-center text-[10px] font-bold uppercase tracking-widest mb-6">Bu pete kaç puan verirsin?</p>
            ...
            <div className="flex justify-between mb-8 px-2">
              {[1, 2, 3, 4, 5].map((p) => (
                <button 
                  ...
                  className={`w-12 h-12 rounded-2xl font-black text-xl flex items-center justify-center transition-all active:scale-90 ${secilenPuan === p ? 'bg-amber-500 text-black shadow-[0_0_20px_rgba(245,158,11,0.5)] scale-110' : 'bg-white/5 border border-white/10 text-white/40'}`}
                >
                  {p}
                </button>
              ))}
            ...
            <h3 className="text-white text-center font-black italic uppercase text-lg mb-1 tracking-tighter">Sence Hangisi?</h3>
            <p className="text-white/40 text-center text-[10px] font-bold uppercase tracking-widest mb-6">Karakterini belirle</p>
            ...
            <div className="grid grid-cols-1 gap-3">
              {[
                ...
                <button 
                  ...
                  className="w-full py-4 rounded-2xl bg-white/5 border border-white/10 text-white font-bold text-sm flex items-center justify-center gap-3 hover:bg-white/10 active:scale-95 transition-all group"
                >
                  {obj.label}
                  <span className="text-[10px] text-amber-500 opacity-0 group-hover:opacity-100 transition-opacity uppercase font-black">Seç ve Onayla →</span>
                </button>
              ))}
            </div>
            ...
            {reklamIzleniyor ? (
              ...
              <div className="space-y-6 flex flex-col items-center">
                ...
                <button 
                  ...
                  className="w-full py-4 rounded-2xl bg-black text-white font-black uppercase italic shadow-lg active:scale-95 disabled:opacity-20 transition-all"
                >
                  Enerji Tazele (+5)
                </button>
              </div>
