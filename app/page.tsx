    ...
    <main className="h-screen w-full bg-black overflow-hidden relative select-none text-white font-sans">
      ...
      <div className="fixed top-0 left-0 w-full z-[60] flex flex-col items-center pt-6 pb-10 bg-gradient-to-b from-black via-black/90 to-transparent">
        <div className="w-full max-w-md flex items-center justify-between px-6 mb-4">
          <div onClick={anaSayfayaDon} className="flex flex-col cursor-pointer active:scale-95 transition-all">
        ...
        <div className="w-full max-w-xs flex justify-around items-center bg-white/5 backdrop-blur-xl p-2 rounded-full border border-white/10 shadow-2xl">
          {['kedi', 'kopek', 'bird', 'hamster', 'reptile'].map((id) => (
            <button key={id} onClick={() => router.push(`/?kat=${id}`)} className={`p-3 rounded-full transition-all active:scale-90 ${kategori === id ? 'text-black scale-110' : 'text-white/40'}`} style={kategori === id ? { backgroundColor: elegantTurkuaz } : {}}>
              ✨
            </button>
          ))}
        </div>
      ...
      {/* FOTOLAR */}
      <div ref={scrollContainerRef} onScroll={(e: any) => setActiveScrollIndex(Math.round(e.target.scrollTop / window.innerHeight))} className="h-screen w-full overflow-y-scroll snap-y snap-mandatory scrollbar-hide">
      ...
      {/* MODALLAR (ÖNCEKİ İLE AYNI MANTIK) */}
      {showLoginModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl">
          <div className="bg-zinc-900 border border-white/10 w-full max-w-sm p-10 rounded-[4rem] shadow-2xl relative text-center">
            ...
            <button onClick={handleGoogleLogin} className="w-full py-5 rounded-2xl font-black uppercase italic tracking-widest flex items-center justify-center gap-3 bg-white text-black active:scale-95 transition-all mt-6">Google Bağlan</button>
          </div>
        </div>
      )}
      ...
      {oylamaPaneli.open && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/90 backdrop-blur-2xl">
          <div className="bg-zinc-900 border border-white/10 w-full max-w-sm p-8 rounded-[4rem] shadow-2xl">
            ...
            <button onClick={oyVer} disabled={secilenPuan === null} className="w-full py-4 rounded-3xl border border-white/10 bg-white/5 text-white font-bold tracking-tight active:scale-95 disabled:opacity-10">Oylamayı Bitir</button>
          </div>
        </div>
      )}
    </main>
